...
- case
  - ![](./image/2026-01-06-19-13-20.png)
  - ![](./image/2026-01-06-19-18-58.png)
  - ![](./image/2026-01-06-19-19-45.png)
  - ![](./image/2026-01-06-19-40-04.png)
...
- case
  - ![](./image/2026-01-06-19-50-28.png)
  - ![](./image/2026-01-06-20-39-32.png)
  - ![](./image/2026-01-06-20-51-47.png)
  - ![](./image/2026-01-06-20-56-53.png)

### 架构

- 分层架构：严格分层架构中，某一层只能调用其下层；松散分层架构中，允许跨层调用。由于UI层和应用层往往都会调用基础设施层，所以大多数系统都是松散分层架构
  - ![](./image/2026-01-15-08-27-59.png)
  - ![](./image/2026-01-15-09-16-02.png)
- 六边形架构：外六边形对应不同端口，内六边形对应应用核心（用例），端口通过适配器与核心交互
  - ![](./image/2026-01-15-09-24-58.png)
- 面向服务架构SOA：将应用划分为多个服务，每个服务对应一个业务功能。在DDD中，每一个领域模型由一组SOA开放服务所包围，这些服务是满足业务目标的。
  - ![](./image/2026-01-16-09-12-16.png)
- RESTful HTTP：基于资源的架构风格，通过HTTP协议进行通信，使用统一的URI标识资源，使用HTTP方法进行操作。
  - 在DDD中，如果将领域模型直接暴露为RESTful资源，会使系统接口变得脆弱，应该为系统接口层单独创建一个限界上下文，通过适当的策略来访问实际的领域模型。
- 数据网织：本质是将分散在不同限界上下文、聚合根中的数据，按照领域语义进行关联、整合和按需组装，避免数据孤岛，同时保持领域模型的纯度。
- 事件驱动架构：通过事件总线实现不同组件之间的解耦通信，适用于高并发和分布式系统
  - ![](./image/2026-01-19-20-08-09.png)
- CQRS架构模式：将读写操作分离，读操作使用专门的查询模型，写操作使用命令模型
  - ![](./image/2026-01-19-19-10-26.png)
...
- case
  - ![](./image/2026-01-07-08-50-23.png)
  - ![](./image/2026-01-07-09-06-19.png)
  - ![](./image/2026-01-07-09-10-22.png)
  - ![](./image/2026-01-07-13-18-16.png)
  - 为什么要避免大范围使用实体？
...
- 层超类型（Layer Supertype）：为某一层中所有类提供通用属性和行为，一般以抽象类的形式存在，作为复用载体。不对应具体业务对象，仅封装层内通用逻辑
...
- case
  - 建模应该使用实体还是值对象？用值对象建模，有助于创建、测试、使用、优化和维护
  - ![](./image/2026-01-08-09-00-54.png)
...
- 将类放在不同模块中，以达到松耦合性和高内聚性。
- 如何保障不同对象之间只存在有限耦合？
- 设计原则
  - ![](./image/2026-01-26-09-22-05.png)
